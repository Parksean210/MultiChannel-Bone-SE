...
3.  **노이즈 (Noise)**
    *   스피치 외에 나머지 소스 슬롯(`S_max-1`)만큼의 공간을 확보합니다.
    *   실제 소리가 들어있는 개수만큼 노이즈 파일을 랜덤으로 뽑고, 나머지는 **0으로 채워진 무음**을 배치합니다.
    *   **형상:** `(S_max-1, 48000)` (고정된 개수의 1D 텐서)

#### 📊 메타데이터 정책 (Metadata & Fixed Batching)
*   **고정 배칭(Fixed Batching)**: 각 샘플마다 노이즈 개수가 다르면 GPU 연산 시 배치를 묶을 수 없습니다. (예: 어떤 건 노이즈 2개, 어떤 건 6개). 이를 해결하기 위해 **가장 많은 소스 기준(예: 7개)으로 텐서 크기를 고정**하고, 남는 자리는 패딩 처리합니다.
*   **8:1:1 분할**: 모든 데이터는 Train(80%), Val(10%), Test(10%)로 엄격히 분할됩니다.

> **💡 결과물 (하나의 샘플):**
> *   `raw_speech`: `(48000,)`
> *   `raw_noises`: `(7, 48000)` (미사용 슬롯은 무음)
> *   `rir_tensor`: `(5, 8, 16000)` (미사용 슬롯은 0)
> *   `noise_ids`: `(7,)` (실제 ID가 들어가며, 미사용 슬롯은 `-1`)
...
CPU가 준비한 샘플들을 **박스(Batch)** 에 담습니다. 모든 텐서의 크기가 패딩을 통해 고정되었으므로, 에러 없이 안정적으로 배치 구성이 가능합니다.

> **📦 배칭 후 텐서 형상:**
> *   `raw_speech`: `(B, 48000)`
> *   `raw_noises`: `(B, 7, 48000)`
> *   `rir_tensor`: `(B, 5, 8, 16000)`
