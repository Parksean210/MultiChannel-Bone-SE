...
*   **Metric Tracking:** `val_loss`, `SI-SDR`, **`DNSMOS` (Overall, Signal, Background)** 등 핵심 성능 지표의 실시간 모니터링
*   **Artifact Storage:** 생성된 오디오 샘플(`.wav`), 모델 체크포인트(`.ckpt`), 설정 파일(`.yaml`)의 중앙 집중식 저장
*   **Model Comparison:** 다양한 모델 아키텍처 및 데이터 처리 기법 간의 정량적 성능 비교
*   **System Tracking:** GPU 사용률, 메모리, 온도 등을 자동 기록하여 성능 병목 지점 파악
...
### 4.4. 음성 향상 지표 해석 가이드 (SE Metrics Interpretation)

| 지표 (Metric) | 좋은 값의 범위 | 의미 및 해석 |
| :--- | :--- | :--- |
| **SI-SDR** | 10dB ~ 20dB+ | 잡음 대비 음성 신호의 강도. 높을수록 원본과 유사하며 잡음이 적음. |
| **DNSMOS (OVRL)** | 3.5 ~ 4.5 | 인간의 인지 품질(MOS) 모사. 4.0 이상이면 매우 깨실한 음질로 평가됨. |
| **STOI** | 0.8 ~ 0.95 | 말소리의 명료도(Intelligibility). 1.0에 가까울수록 정보 손실이 없음. |
| **PESQ** | 3.0 ~ 4.5 | 음향 신호의 품질(Quality). 광대역(WB) 기준 3.5 이상이면 우수한 품질. |
...
본 프로젝트(`speech_enhancement`)는 음성 향상 연구에 특화된 추가 모니터링 기능을 제공합니다.

### 7.1. 스펙트로그램 시각화 (Spectrograms)
오디오 파일(`Noisy`, `Enhanced`, `Target`)과 함께 주파수 특성을 보여주는 고해상도 이미지가 자동으로 로깅됩니다.
- **확인 방법**: MLflow UI -> Run Detail -> `Artifacts` -> `audio_samples/` 폴더
- **특징**: Y축이 실제 주파수(kHz)로 표시되어 잡음 제거 및 잔향 제거 효과를 직관적으로 파악할 수 있습니다.

### 7.2. GPU 및 시스템 메트릭 자동 로깅
`main.py` 실행 시 `mlflow.enable_system_metrics_logging()`이 호출되어 별도 설정 없이도 하드웨어 상태(CPU, GPU, RAM, Disk)가 기록됩니다.

### 7.3. 설정 가이드 (Configuration)
`configs/ic_conv_tasnet.yaml`에서 로깅 관련 설정을 관리합니다. `main.py`가 슬림화됨에 따라 콜백 등록도 YAML에서 수행합니다.
...
trainer:
  logger:
    ...
    init_args:
      experiment_name: "Speech_Enhancement_Research"
      run_name: "Base_Experiment_01"
  callbacks:
