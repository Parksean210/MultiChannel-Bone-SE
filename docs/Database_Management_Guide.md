# 🗄️ 데이터베이스 관리 가이드 (Database Management Guide)

이 문서는 본 프로젝트에서 사용하는 **SQLite 데이터베이스(`data/metadata.db`)**를 관리하는 방법을 아주 상세히 설명합니다.
수십만 개의 오디오 파일 경로와 메타정보(길이, 화자, 언어 등)를 DB에 저장해두고, 학습 시에 이를 고속으로 조회하여 사용합니다.

---

## 🚀 1. 가장 쉬운 사용법 (CLI 도구)

복잡한 코드 없이, 터미널 명령어로 데이터를 추가할 수 있습니다. 스크립트 위치는 `scripts/manage_db.py`입니다.
모든 명령어는 `uv run`을 사용하여 실행하는 것을 권장합니다.

### 📊 데이터베이스 현황 확인 (Stats)
현재 DB에 저장된 파일 개수와 카테고리별 분포, **샘플 레이트 현황**을 한눈에 확인할 수 있습니다.
```bash
uv run python3 scripts/manage_db.py stats
```

### 🔄 경로 자동 동기화 (Sync)
`.wav` 파일을 `.npy`로 변환했거나 파일 위치를 옮겼을 때, DB에 저장된 경로를 실제 파일과 일치하도록 자동 업데이트합니다.
```bash
uv run python3 scripts/manage_db.py sync
```

### 🔊 노이즈 데이터 추가하기
새로운 노이즈 데이터를 다운로드 받거나 녹음했다면, 다음 명령어로 DB에 등록하세요. 사용자가 직접 카테고리를 지정할 수 있어 관리가 용이합니다.

**기본 명령어 포맷:**
```bash
uv run python3 scripts/manage_db.py noise \
    --path "[폴더 경로]" \
    --dataset "데이터셋이름" \
    --category "[대분류]" \
    --sub "[소분류]"
```

**실전 예시 (심볼릭 링크 활용):**
D 드라이브에 있는 `Living_Noise` 폴더를 프로젝트로 연결해 등록하는 경우:

1. **심볼릭 링크 생성**:
   ```bash
   ln -s "/mnt/d/New_Noise_Data" data/raw/noise/new_living_noise
   ```

2. **DB에 등록**:
   ```bash
   uv run python3 scripts/manage_db.py noise \
       --path data/raw/noise/new_living_noise \
       --dataset "LivingNoise_v1" \
       --category "생활소음" \
       --sub "청소기"
   ```

### 🗣️ 음성(Speech) 데이터 추가하기
KsponSpeech 같은 대용량 음성 데이터를 추가할 때 사용합니다. 화자(Speaker)와 언어(Language) 정보를 명시할 수 있습니다.

```bash
uv run python3 scripts/manage_db.py speech \
    --path data/speech/KsponSpeech \
    --dataset KsponSpeech \
    --language "ko" \
    --speaker "Kspon_0001" 
    # speaker 생략 시 폴더명에서 자동 추론
```

### 🏛️ RIR (공간 음향) 데이터 추가하기
시뮬레이션으로 생성된 RIR 파일(`.wav` 또는 `.pkl`)이 있는 폴더를 통째로 등록합니다.

```bash
uv run python3 scripts/manage_db.py rir \
    --path data/rirs \
    --dataset "Generated_RIR" \
    --split "train"
```

---

## �️ 데이터 분할 및 고정 정책 (Split Policy) - **중요**

### 1. 데이터 분할 재배치 (`realloc`)
DB에 등록된 데이터를 Train:Val:Test (기본 8:1:1) 비율로 나눕니다.

```bash
uv run python3 scripts/manage_db.py realloc --type speech --ratio 0.8 0.1 0.1
uv run python3 scripts/manage_db.py realloc --type noise --ratio 0.8 0.1 0.1
uv run python3 scripts/manage_db.py realloc --type rir --ratio 0.8 0.1 0.1
```

### 2. 고정 분할 정책 (Fixed Split Policy)
본 시스템은 데이터셋 확장 시의 **데이터 누수(Data Leakage)**를 방지하기 위해 점진적 분할 방식을 채택하고 있습니다.

- **작동 원리**: `realloc` 명령어 실행 시, **이미 `val`이나 `test`로 지정된 데이터는 절대 `train`으로 돌아가지 않습니다.** (Anchoring)
- **효과**: 새로운 데이터를 추가하고 다시 `realloc`을 돌려 파티션을 확장하더라도, 기존에 구축된 검증/테스트 환경이 일관되게 유지됩니다. 즉, 기존 모델 평가 결과의 신뢰성을 보장합니다.

---

## 💻 2. 파이썬 코드에서 사용하기 (API)

Jupyter Notebook이나 다른 스크립트에서 직접 DB를 조작하고 싶다면 `DatabaseManager`를 import해서 쓰세요.

```python
from src.db import create_db_engine, DatabaseManager

# 1. DB 연결 (엔진 생성)
engine = create_db_engine("data/metadata.db")

# 2. 매니저 초기화
manager = DatabaseManager(engine)

# 3. 데이터 등록
# 노이즈 추가
manager.index_noise(
    root_dir="data/raw/noise/server_room", 
    dataset_name="ServerNoise",
    category="machine",
    sub_category="fan"
)

# 음성 추가
manager.index_speech(
    root_dir="data/raw/speech/new_speaker",
    dataset_name="MyCustomVoice",
    language="en"
)
```

---

## 🏗️ 3. 데이터 구조 (Schema)

DB 내부 스키마(`src/data/models.py`)는 다음과 같습니다.

### Table: `speechfile`
| 필드명 | 설명 | 예시 |
| :--- | :--- | :--- |
| `id` | 고유 번호 | 1 |
| `path` | 파일 절대 경로 (Unique) | `/home/user/data/speech/file.wav` |
| `dataset_name` | 데이터셋 이름 | `KsponSpeech` |
| `speaker` | 화자 식별자 | `Kspon_0001` |
| `language` | 언어 코드 | `ko` |
| `duration_sec` | 오디오 길이 (초) | 4.52 |
| `sample_rate` | 샘플 레이트 (Hz) | 16000 |
| `split` | 분할 영역 | `train` |

### Table: `noisefile`
| 필드명 | 설명 | 예시 |
| :--- | :--- | :--- |
| `id` | 고유 번호 | 1 |
| `path` | 파일 절대 경로 (Unique) | `/home/user/data/noise/car.wav` |
| `dataset_name` | 데이터셋 이름 | `UrbanSound` |
| `category` | 대분류 (사용자 입력) | `교통수단` |
| `sub_category` | 소분류 (사용자 입력) | `차량소음` |
| `duration_sec` | 오디오 길이 (초) | 10.0 |
| `sample_rate` | 샘플 레이트 (Hz) | 16000 |
| `split` | 분할 영역 | `train` |

### Table: `rirfile`
| 필드명 | 설명 | 예시 |
| :--- | :--- | :--- |
| `id` | 고유 번호 | 1 |
| `path` | 파일 절대 경로 (Unique) | `/home/user/data/rirs/rir_00001.pkl` |
| `dataset_name` | 데이터셋 이름 | `SimulatedRIR` |
| `room_type` | 방 형태 | `shoebox` |
| `num_noise` | 노이즈 소스 개수 | 4 |
| `num_mic` | 마이크 개수 | 4 |
| `num_bcm` | 골전도 센서 개수 | 1 |
| `rt60` | 잔향 시간 (초) | 0.35 |
| `duration_sec` | RIR 길이 (초) | 2.0 |
| `split` | 분할 영역 | `train` |

---

## ❓ 자주 묻는 질문 (FAQ)

**Q. 노이즈 폴더에 파일 하나만 추가했습니다. 전체를 다시 인덱싱해야 하나요?**  
A. 네, `manage_db.py` 명령어를 똑같이 다시 실행하시면 됩니다.  
내부에서 **"이미 DB에 있는 경로는 무시(Skip)"**하도록 짜여 있어서, **새로 추가된 파일만 쏙 골라서** 빠르게 등록됩니다.

**Q. DB 파일을 실수로 지웠습니다!**  
A. 걱정 마세요. `data/metadata.db` 파일은 언제든 다시 만들 수 있습니다.  
`final_indexing.py` 스크립트나 개별 명령어를 다시 실행하면 파일들을 스캔하여 DB를 새로 구축해 줍니다.

---
**최종 수정일**: 2026-02-05
